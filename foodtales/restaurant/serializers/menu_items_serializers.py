...
class MenuSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if Menu.objects.filter(restaurant=restaurant, date=date).exists():
            raise serializers.ValidationError(
                "A menu for this date already exists for your restaurant. Create a new menu for a different date or with different restaurant."
            )
    ...
    def update(self, instance, validated_data):
        ...
        if items_data is not None:
            ...
            menu.items.exclude(
                id__in=[item.get("id") for item in items_data if item.get("id")]
            ).delete()

            for item_data in items_data:
                item_id = item_data.get("id")
                if item_id:
                    MenuItem.objects.filter(id=item_id, menu=menu).update(**item_data)
                else:
                    MenuItem.objects.create(menu=menu, **item_data)
